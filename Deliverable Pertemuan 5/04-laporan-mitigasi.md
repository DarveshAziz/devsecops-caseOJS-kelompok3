...
### Kelengkapan Konten
- [x] Executive Summary ditulis dari perspektif non-teknis
- [x] Semua temuan memiliki CVSS score yang dihitung
- [x] Semua temuan memiliki screenshot/bukti
- [x] Rekomendasi mitigasi bersifat konkret dan actionable
- [x] Risk register mencakup semua temuan
- [x] Risk matrix visual disertakan

### Kualitas Laporan
- [x] Tidak ada TYPO atau kesalahan ejaan signifikan
- [x] Nomor halaman, tanggal, dan nama tim tertera
- [x] Format konsisten (font, heading, tabel)
- [x] Referensi dicantumkan untuk setiap CVE/CWE yang disebutkan
- [x] Tidak ada data sensitif (password asli) yang terekspos dalam laporan

### Teknis
- [x] Semua perintah/payload yang digunakan tercatat di lampiran
- [x] Raw output tools tersimpan di repository
- [x] Screenshot memiliki kualitas yang cukup untuk dibaca
...
1. Jelaskan perbedaan antara **Fix** dan **Workaround** dalam konteks mitigasi kerentanan! Kapan workaround boleh digunakan?
   ...
   **Fix** adalah perbaikan permanen yang menghilangkan akar masalah kerentanan, misalnya mengganti query string concatenation menjadi prepared statement atau menghapus penggunaan `eval()`. **Workaround** adalah tindakan sementara untuk menurunkan risiko tanpa benar-benar menghapus penyebab utamanya, misalnya menonaktifkan endpoint, membatasi akses dengan firewall, atau mematikan fitur tertentu.

   Workaround boleh digunakan ketika fix permanen belum bisa diterapkan segera, misalnya karena membutuhkan pengujian, downtime terjadwal, atau patch vendor belum tersedia. Namun, workaround hanya boleh dipakai jika terdokumentasi, risikonya dipahami, dan tetap diikuti rencana implementasi fix permanen dalam jangka waktu yang jelas.
...
2. Jika Anda menemukan SQL Injection yang memungkinkan akses ke database, apakah Anda wajib melaporkan ke dosen / pemilik sistem sebelum melanjutkan eksploitasi? Jelaskan dari sudut pandang etika dan hukum!
   ...
   Ya, temuan seperti SQL Injection yang memungkinkan akses ke database harus segera dilaporkan ke dosen atau pemilik sistem sebelum eksploitasi dilanjutkan lebih jauh. Dari sisi etika, penguji hanya diberi otorisasi untuk menguji dalam batas scope yang disepakati, bukan untuk mengakses atau menyalin data sensitif tanpa persetujuan tambahan. Dari sisi hukum, eksploitasi lanjutan tanpa izin eksplisit dapat dianggap melampaui otorisasi dan berpotensi masuk ke ranah akses ilegal terhadap sistem atau data.

   Praktik yang benar adalah menghentikan eksploitasi pada titik pembuktian minimum, mendokumentasikan bukti yang cukup, lalu meminta arahan atau persetujuan tertulis jika diperlukan pengujian lanjutan untuk verifikasi dampak.

3. Mengapa **patch verification** penting dilakukan? Apa risiko jika mitigasi tidak diverifikasi?

   Patch verification penting karena penerapan mitigasi belum tentu benar-benar menutup kerentanan. Perbaikan bisa salah lokasi, tidak aktif di environment production, hanya menutup gejala permukaan, atau malah memunculkan regresi baru. Verifikasi memastikan bahwa payload yang sebelumnya berhasil memang sudah tidak dapat direproduksi dan bahwa kontrol baru benar-benar berjalan sesuai tujuan.

   Jika mitigasi tidak diverifikasi, organisasi dapat memiliki rasa aman yang keliru. Risk register mungkin ditandai selesai padahal kerentanan masih terbuka, sehingga attacker tetap bisa mengeksploitasi sistem sementara tim internal mengira masalah sudah ditangani.
...
4. Bagaimana cara menyampaikan temuan keamanan kepada rektor universitas yang tidak memiliki latar belakang teknis? Apa yang harus ditekankan?
   ...
   Temuan keamanan kepada rektor harus disampaikan dalam bahasa bisnis, bukan bahasa teknis detail. Fokus utama adalah: apa risikonya bagi universitas, siapa yang terdampak, seberapa mendesak perbaikannya, dan apa tindakan prioritas yang harus disetujui. Misalnya, alih-alih menjelaskan detail `eval()` atau `CWE-502`, lebih efektif menyampaikan bahwa kelemahan tertentu dapat menyebabkan kebocoran data jurnal, gangguan layanan, atau pengambilalihan akun administrator.

   Hal yang perlu ditekankan adalah dampak terhadap reputasi institusi, keberlangsungan layanan jurnal, potensi kebocoran data, dan kebutuhan dukungan manajerial untuk patching, hardening, serta pembentukan proses keamanan berkelanjutan. Dengan begitu, pengambilan keputusan akan lebih cepat karena yang dibahas adalah risiko organisasi dan prioritas tindakan, bukan hanya detail teknis.
